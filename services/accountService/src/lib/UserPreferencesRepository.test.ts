import { deleteUser, getUserByID, upsertUser, UserInfo } from './UserPreferencesRepository';
...
describe('upsertUser', () => {
  it('updates existing user when given an existing user in the database', async () => {
    client.query(
      `INSERT INTO user_preferences VALUES ('1A2B3C', 'User', 'email@gmail.com', true, true);`,
    );
    ...
    upsertUser(userInfo);
    expect(getUserByID('1A2B3C')).toBe(userInfo);
  ...
  it('adds user if they do not exist in the database ', async () => {
    ...
    upsertUser(userInfo);
    expect(getUserByID('Test1')).toBe(userInfo);
