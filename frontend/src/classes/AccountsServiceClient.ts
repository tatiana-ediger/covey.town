...
export default class AccountsServiceClient {
    ...
    async saveUser(requestData: SaveUserRequest): Promise<SaveUserResponse> {
      const responseWrapper = await this._axios.post<ResponseEnvelope<SaveUserResponse>>('/user', requestData);
    ...
    async deleteUser(requestData: DeleteUserRequest): Promise<void> {
      const responseWrapper = await this._axios.delete<ResponseEnvelope<void>>(`/user/${requestData.userID}`);
      return AccountsServiceClient.unwrapOrThrowError(responseWrapper);
    }
  }
